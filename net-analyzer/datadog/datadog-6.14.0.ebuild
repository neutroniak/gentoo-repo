...
src_compile() {
	...
	GOPATH="${S}" go build -race -a -o 'bin/agent/agent' \
		-ldflags "-X ${REPO_PATH}/pkg/version.AgentVersion=${PV} -X ${REPO_PATH}/pkg/serializer.AgentPayloadVersion=${AGENT_PAYLOAD_VERSION} -X ${REPO_PATH}/pkg/collector/py.pythonHome=/usr -r /usr/lib64 -X ${REPO_PATH}/pkg/version.Commit=748545e4d -X ${REPO_PATH}/pkg/version.AgentVersion=6.14.0 "  \
...
#	GOPATH="${S}" go run 'github.com/DataDog/datadog-agent/cmd/trace-agent'
